...
Img::Img() : Img(0, {}) { };
Img::Img(unsigned int l , Matrix data) : label{l}, img_data{data} { };


std::vector<Img> csv_to_imgs(const char* file_name, int number_of_images, ULL image_length = 0, ULL image_height = 0) {
  ...
  for (int i = 0; !INPUT.eof() && i < number_of_images; ++i) {
    Img curr_img;
    ...
    long double img_label = *image_pixels.begin() * 256.0;      // + Added 256, cause it was divided by it in prev-step
    image_pixels.erase(image_pixels.begin());
    curr_img.label = img_label;
    std::cout << "THEORETICAL LABEL: " << img_label << " | LABEL: " << curr_img.label << "\n";
    ...
    mat.set_as(image_pixels);                           // ERROR HEWRE!
